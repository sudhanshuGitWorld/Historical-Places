...
const Header = ({ title, data }) => {
    ...
    const placeStore = useSelector(state => state.places.data);
    const isRecommendedShow = useSelector((state) => state.places.isRecommendedShow);

    const randomSuggestion = (path) => {
      if (path === 'home') {
        const randomSuggestionData = [];
        dispatch(showRandomSuggestion({ randomSuggestionData, isRecommendedShow: true }));
      } else {
        const suggestionId = Math.floor(Math.random() * data?.length+1);
        const randomSuggestionData = placeStore?.filter(item => item.id === suggestionId.toString());
        dispatch(showRandomSuggestion({ randomSuggestionData, isRecommendedShow: true }));
      }
    }
      ...
      <>
        <header>
          <nav className="flex bg-blue-400 border-gray-200 px-4 lg:px-6 py-2.5">
            <div
              className="flex flex-wrap justify-between items-center cursor-pointer ml-0 w-5/12 lg:w-[14%] md:w-1/5"
              onClick={() => randomSuggestion("home")}
            >
